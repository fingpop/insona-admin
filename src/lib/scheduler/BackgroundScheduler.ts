...
async function cleanupOldEvents() {
  ...
  if (result.count > 0) {
    logger.info("Scheduler", `清理过期事件: ${result.count} 条（>${EVENT_RETENTION_DAYS} 天）`);
  }
}
...
async function refreshDeviceAliveForAllGateways() {
  ...
  for (const gw of gateways) {
    try {
      ...
      logger.debug("Scheduler", `Gateway ${gw.id}: 刷新设备在线状态 refreshed=${result.refreshed}, online=${result.online}, offline=${result.offline}`);
    } catch (err) {
      logger.error("Scheduler", `Gateway ${gw.id} 刷新设备在线状态失败:`, (err as Error).message);
    }
  }
}
...
export function startScheduler() {
  ...
  logger.info("Scheduler", "启动后台调度服务...");

  // 立即执行一次
  runSchedulerTick().then((result) => {
    logger.info("Scheduler", `初始检查完成: executed=${result.executed}, errors=${result.errors}`);
  }).catch((err) => {
    logger.error("Scheduler", "初始检查失败:", err);
  });
  ...
  schedulerInterval = setInterval(async () => {
    try {
      ...
      if (result.executed > 0 || result.errors > 0) {
        logger.info("Scheduler", `定时检查: executed=${result.executed}, errors=${result.errors}, skipped=${result.skipped}`);
      }
    } catch (err) {
      logger.error("Scheduler", "定时检查失败:", err);
    }
  }, 60_000);

  logger.info("Scheduler", "调度服务已启动，每 60 秒检查一次");
  ...
  deviceAliveRefreshInterval = setInterval(async () => {
    try {
      ...
      logger.error("Scheduler", "设备在线状态定时刷新失败:", err);
    }
  ...
  setTimeout(() => {
    refreshDeviceAliveForAllGateways().catch((err) => {
      logger.error("Scheduler", "设备在线状态首次刷新失败:", err);
    });
  }, 30_000);

  logger.info("Scheduler", `设备在线状态刷新已启动，每 ${DEVICE_ALIVE_REFRESH_INTERVAL / 1000} 秒执行一次`);
  ...
  eventCleanupInterval = setInterval(async () => {
    try {
      ...
      logger.error("Scheduler", "事件定时清理失败:", err);
    }
  }, EVENT_CLEANUP_INTERVAL);

  // 启动时立即执行一次清理
  cleanupOldEvents().catch((err) => {
    logger.error("Scheduler", "事件初始清理失败:", err);
  });

  logger.info("Scheduler", `事件清理已启动，每 ${EVENT_CLEANUP_INTERVAL / 60000} 分钟执行一次，保留 ${EVENT_RETENTION_DAYS} 天`);
}
